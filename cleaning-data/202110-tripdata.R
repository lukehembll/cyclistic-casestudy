...
```{r}
tripdata <- read_csv("/cloud/project/202110-divvy-tripdata.csv")
```
...
* total rows in dataset is 631226

#### ride_id

* no missing data
* every row is unique (no duplicates) with 631226 id's
...
* no missing data
* datetime value
* all dates are after 2021-10-01

#### ended_at

* no missing data
* datetime value
* there are dates up to 2021-11-03

#### start_station_name

* there are 108210 missing values
* 793 unique stations
...
* there are 114834 missing values
* 790 unique stations
...
```
* 153,254 records with missing data
...
```
* there are 6 rows here where the trip duration is a negative value or equal to 0
...
#### Export the data
```{r}
write.csv(tripdatafinal, "202110_tripdata_cleaned.csv", row.names=FALSE)
```
