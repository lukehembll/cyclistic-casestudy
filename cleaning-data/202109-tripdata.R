...
```{r}
tripdata <- read_csv("/cloud/project/202109-divvy-tripdata.csv")
```
...
* total rows in dataset is 756147

#### ride_id

* no missing data
* every row is unique (no duplicates) with 756147 id's
...
* no missing data
* datetime value
* all dates are after 2021-09-01

#### ended_at

* no missing data
* datetime value
* there are dates up to 2021-10-01

#### start_station_name

* there are 93113 missing values
* 758 unique stations

#### start_station_id

* there are 93111 missing values
* 758 unique id's
* id's are inconsistent in naming and length

#### end_station_name

* there are 99261 missing values
* 756 unique stations
...
```
* 134,997 records with missing data
...
```
* there are 47 rows here where the trip duration is a negative value or equal to 0
...
#### Export the data
```{r}
write.csv(tripdatafinal, "202109_tripdata_cleaned.csv", row.names=FALSE)
```
